	...
	if stat("package.json") ~= nil then
		...
		if tab.workspaces ~= nil and tab.workspaces.packages ~= nil then
			...
			for _, pkg_glob in pairs(pkg_globs) do
				for _, filename in ipairs(vim.fn.glob(pkg_glob .. "/package.json", true, true)) do
...
M.setup = function(config)
  ...
  project_level_config_filename = config.neoscopes_config_filename or 'neoscopes.config.json'
	local project_level_config = get_project_level_config(project_level_config_filename)
	...
	if config.enable_scopes_from_npm  or project_level_config.enable_scopes_from_npm then
	...
	if config.diff_branches_for_scopes then
		local git_scopes = get_scopes_from_git_diffs(config.diff_branches_for_scopes)
    M.add_all(git_scopes)
	end
	if project_level_config.diff_branches_for_scopes then
		local git_scopes = get_scopes_from_git_diffs(project_level_config.diff_branches_for_scopes)
    M.add_all(git_scopes)
	end
...
M.add = function(scope)
	...
	elseif type(scope.files) ~= "table" then
		error("scope.files must be a table of individual files to include in the scope (it is okay to be empty)")
	end
